...
#include <iostream>
#include "../tool/include/debug/camera_parms_debug.h"
using namespace std;
namespace camera
{
    ...
    void HikCamera::Init(bool debug_flag, const std::string &config_file_path, const std::string &intrinsic_para_file_path, std::chrono::_V2::steady_clock::time_point timeStart)
    {
        ...
        //********** 设置采集像素格式 **********/
        nRet = MV_CC_SetEnumValue(handle, "PixelFormat", PixelType_Gvsp_BGR8_Packed); // 目前 BGR
        if (MV_OK == nRet)
        {
        ...
        nRet = pthread_mutex_init(&mutex, nullptr); // 初始化互斥量
        if (nRet != 0)
        {
        ...
        nRet = pthread_create(&nThreadID, nullptr, HKWorkThread, &HKWorkParam1); // 开启取图线程
        if (nRet != 0)
        {
    ...
    HikCamera::~HikCamera()
    {
        ...
        if (MV_OK != ret)
        {
            printf("MV_CC_StopGrabbing fail! ret [%x]\n", ret);
            return;
        }
        ...
        if (MV_OK != ret)
        {
            printf("MV_CC_CloseDevice fail! ret [%x]\n", ret);
            return;
        }
        ...
        if (MV_OK != ret)
        {
            printf("MV_CC_DestroyHandle fail! ret [%x]\n", ret);
            return;
        }
    ...
    //^ ********************************** 相机取图 ************************************ //
    bool HikCamera::ReadImg(cv::Mat &image)
    {
