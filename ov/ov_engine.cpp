...
{
    // 读取配置文件
    std::string OVInferDevice;
    //std::string OVInferModel;
    cv::FileStorage fs("../control/aim_config.yaml", cv::FileStorage::READ);
    fs["OVInferDevice"] >> OVInferDevice;
    //fs["OVInferModel"] >> OVInferModel;
    fs.release();
    ...
    network = core.read_model("../model/OpenVINO/2023-04-16-best.xml");
    ...
    executable_network = core.compile_model(network, OVInferDevice); // SET DEVICE
