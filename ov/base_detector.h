...
public:
    ...
    int PostProcessYoloV5n(cv::Mat &srcimg, const float threshold, const std::vector<float *> inputData, std::vector<DetectObject> &objects);

    std::vector<DetectObject> run(cv::Mat &img);
    ...
    COLOR color_judge(cv::Rect &rect, cv::Mat &img)
    {
            ...
            0 <= rect.height && rect.y + rect.height <= img.rows)
        {
            ...
            color_id_ = sum_r > sum_b ? COLOR::RED_OV : COLOR::BLUE_OV;
            return color_id_;
        }
        else
        {
            return COLOR::GRAY_OV;
        }
    }
