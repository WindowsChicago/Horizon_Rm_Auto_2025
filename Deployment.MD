## Horizon_Rm_Auto_NT 环境部署文档

###### by ZYL

##### 系统：Ubuntu20/22 LTS

##### 平台：带N卡的PC/NX/AGX(TRT模式)，任意AMD64/ARM64设备(OpenVINO模式)
...
cuda 11.8/11.4（TRT必需）
cudnn 8.9.7.29（TRT必需）
TensorRT 8.5.1.7（TRT必需）
OpenVINO2022及以上（OV必须）
Eigen 3.3.7/3.4.0
Ceres 1.14/2.20
fmt 10.2.0/8.1.1
yaml-cpp 1.2
MVS_STD_GML_V2.1 (海康相机SDK)
Galaxy_Linux-x86_Gige-U3_32bits-64bits_1.5（大恒相机SDK）
linuxSDK_V2.1.0.37（迈德相机SDK）
```
...
**注意：Ubuntu22可能在cmake .. 时出现如下报错：**

```
CMake Error at cmake/findtbb.cmake:187 (file):
file failed to open for reading (No such file or directory): /usr/include/tbb/tbb_stddef.h
```

原因是Ubuntu22默认apt下载的TBB是2021.5版本，而TBB在2021.1版本之后移除了tbb_stddef.h文件，可以直接生成一个空文件顶替，命令：
...
迈德驱动（linuxSDK_V2.1.0.37）：直接运行解压目录下安装脚本：
...
```
cmake -DYAML_BUILD_SHARED_LIBS=on ..  （-选项表示生成共享库）
make -j12
sudo make install
...
```
./trtexec --onnx=/home/ad/xxxx.onnx --saveEngine=/home/ad/xxxx.engine
```

注意：该命令在Ubuntu 22下不能加上sudo，否则可能报错：error while loading shared libraries: libnvinfer_plugin.so.8

**19.**关于ssh图形化调试的问题：打开文件-三条杠，勾选显示隐藏文件便可在主目录看见隐藏的.bashrc文件，双击使用文件编辑器打开或命令：
...
**20.**编译调试：选择对应设备的CMakeLists，把其名称改为“CMakeLists.txt”,进入control目录，修改define.h选择相机和加速推理模式（OV/TRT），修改debug.yaml修改使用模型，识别颜色和其他参数，修改rune_config.yaml修改打符相关参数，回到根目录新建build文件夹，进入build执行命令：
